...
if ! $BOOTMODE; then
  ...
  ui_print " " 
  ui_print " Flash in magisk if you wanna Install it" 
  ui_print "  " 
  ...
  ui_print " Uninstalled Successfully" 
  exit 0
fi
...
tar -xf $MODPATH/utils/other -C $MODPATH/utils/ 2>/dev/null
tar -xf $MODPATH/utils/backup -C $MODPATH/system/product/overlay/ 2>/dev/null
